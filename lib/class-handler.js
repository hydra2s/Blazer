...
export class ClassHandler {
  ...
  get (target, name) {
    ...
    if (name == "$serialize") { return async()=>JSOX.parse(await self.get(target.className, name)()); } else
  ...
  apply(target, thisArg, args) {
    ...
    return wrap((async()=>{
      if (target.last) { await target.last; }; target.last = null; // await last action
      return (await self.apply(target.className, null, await Promise.all(args)));
    })());
  }
}
