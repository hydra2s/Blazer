...
export class Protocol {
  ...
  async handleEvent(json, shared) {
    ...
    if ((isRetrive && hasKey) || (isOperate && classObj)) {
      ...
      let got = undefined, exception = undefined;
...
`;
          ...
          case "delete":
            ...
            break;
          case "apply":
            got = await Reflect.apply(classObj.value, (await this.handleResult(await thisArgRaw, await aSet(shared))) || classObj.objParent, await args()); if (temporary) { classObj.value = undefined; }; hasResult = true; temporary = true;
            if (shared) shared.splice(0, shared.length);
            break;
          case "construct":
            got = await this.makeClass(await Reflect.construct(classObj.value, await args())); if (temporary) { classObj.value = undefined; }; hasResult = true; temporary = true;
            if (shared) shared.splice(0, shared.length);
            break;
          case "get":
...
`//);
    ...
    //
    return {id, type, result, error, hasResult, className, methodName, hasKey, isOperate, temporary};
  }
