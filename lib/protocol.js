...
export class Protocol {
  ...
  async handleArgument(a, payload={}) { a = await a;
    ...
    if (typeOf != "undefined" && a !== null) {
      ...
      if (typeOf == "function" || data instanceof Function || (!a?.$shared && typeOf == "object" && isInstance && !isUnwrap) || (data?.$isProxy || data?.$isClass)) {
        const _got_ = data;
        if (payload.temporary || this.instances.indexOf(_got_) >= 0 || (_got_?.$isProxy || _got_?.$isClass)) {
  ...
  async handleEvent(result, shared) {
    let {type, thisArgRaw, id, temporary, className, methodName, argsRaw, error} = {
      type: "unknown", temporary: false, thisArgRaw: null, id: "", className: "", methodName: "", argsRaw: [], error: null,
      ...result
    };
    type ||= "unknown";
...
`//);
    ...
    //
    return {...(result||{}), error, hasResult, className, methodName, hasKey, isOperate, temporary, id, type};
  }
