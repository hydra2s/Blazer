...
export class Protocol {
  ...
  handleResult(a, shared) {
    ...
    if (a.type == "array" || data instanceof Array) { data = data.map(($a)=>this.handleResult($a, shared)); };
    if (a.type == "object") {
  ...
  async handleArgument(a, payload={}) { a = await a; 
    ...
    if (data instanceof Array) { typeOf = "array"; data = await Promise.all((await Promise.all(data)).map(async($a)=>await this.handleArgument(await $a, payload))); }
  
    if (typeOf == "function" || (typeOf == "object" && isInstance) || (a?.$isProxy || a?.$isClass)) {
      typeOf = "proxy";
      this.register(data = uuidv4(), a, !payload.temporary);
    }

    if (typeOf == "object" && a?.$unwrap && typeof a?.$unwrap != "function") {
  ...
  async encodeArguments(args, shared) {
    return await Promise.all(args.map((a)=>this.handleArgument(a, {temporary: true, shared})));
  }
