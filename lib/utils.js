...
const loadImageBitmap = async (bitmap, offscreen = false) => {
    ...
    var canvas = createDOMCanvas(bitmap.width, bitmap.height);//(offscreen || isWorker) ? new OffscreenCanvas(bitmap.width, bitmap.height) : createDOMCanvas(bitmap.width, bitmap.height);
...
export {
    createDOMCanvas, loadImageBitmap, blobToArrayBuffer, createOffscreenCanvas, toBlob, encodeURL, concat, saveBlob, loadBitmapAsBlob, provideForWorker, loadBitmapThroughput, uuidv4, loadImage, swap32, encodeSvg, getSharedImageData, wrap
}
