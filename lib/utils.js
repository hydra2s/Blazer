...
//
const loadImageBitmap = async (bitmap, offscreen = false) => {
    const isWorker = typeof WorkerGlobalScope !== 'undefined' && self instanceof WorkerGlobalScope;
    var bitmap = await bitmap;
    var canvas = (offscreen || isWorker) ? new OffscreenCanvas(bitmap.width, bitmap.height) : createDOMCanvas(bitmap.width, bitmap.height);
...
const blobToArrayBuffer = async(url) => {
    const $url = await url;
    return await fetch(($url instanceof Blob) ? URL.createObjectURL($url) : $url).then(res => res.arrayBuffer())
}
...
export {
    loadImageBitmap, blobToArrayBuffer, createOffscreenCanvas, toBlob, encodeURL, concat, saveBlob, loadBitmapAsBlob, provideForWorker, loadBitmapThroughput, uuidv4, loadImage, swap32, encodeSvg, getSharedImageData, wrap
}
