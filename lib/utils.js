...
const loadImageBitmap = async (bitmap, createFunc = createDOMCanvas, showFunc = (C,B)=>{
    //C.getContext("bitmaprenderer").transferFromImageBitmap(B);
...
const getSharedImageData = (ctx, x, y, w_or_image, h) => {
    ...
    const shared = (w_or_image.data?.buffer || w_or_image.data) || new SharedArrayBuffer(w*h*4);
    for (let Y=0;Y<h;Y++) {
...
export {
    _LOG_, createDOMCanvas, loadImageBitmap, blobToArrayBuffer, createOffscreenCanvas, toBlob, encodeURL, concat, saveBlob, loadBitmapAsBlob, provideForWorker, loadBitmapThroughput, uuidv4, loadImage, swap32, encodeSvg, getSharedImageData, wrap
}
