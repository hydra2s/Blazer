export class CommandEncoder {
  ...
  async set(className, methodName, value) {
    ...
    let pt = this.pt;
    return { type: "set", className, methodName, argsRaw: await pt.encodeArguments([value]) };
  }
  ...
  async apply(className, methodName, args, thisArg, _shared_) {
    ...
    methodName = (typeof methodName == "string" || methodName instanceof String) ? methodName : "";
    let thisArgRaw = await pt.handleArgument(await pt.makeClass(thisArg), {shared: _shared_, className, methodName});
    return { type: "apply", className, methodName, thisArgRaw, argsRaw: await pt.encodeArguments(args, _shared_) };
  }

  async construct(className, args, _shared_) {
    let pt = this.pt;
    return { type: "construct", className, argsRaw: await pt.encodeArguments(args, _shared_) };
  }
}
