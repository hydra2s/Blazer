export class CommandEncoder {
  ...
  apply(className, methodName, args, thisArg, _shared_) {
    let pt = this.pt;
    args = Array.isArray(methodName) ? methodName : args;
    methodName = (typeof methodName == "string" || methodName instanceof String) ? methodName : "";
    let thisArgRaw = pt.handleArgument(pt.makeClass(thisArg));
    return { type: "apply", className, methodName, thisArgRaw, argsRaw: pt.encodeArguments(args, _shared_) };
  }

  construct(className, args, _shared_) {
    let pt = this.pt;
    return { type: "construct", className, argsRaw: pt.encodeArguments(args, _shared_) };
  }
}
