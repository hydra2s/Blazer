...
//import pako from "../pako/pako.esm.mjs";
    ...
    recode(binPNG) {
        ...
        this.reader.chunks = this.reader.chunks.filter((chunk)=>{
            return chunk.name == "IHDR" || 
                   chunk.name == "IDAT" || 
                   chunk.name == "IEND" || 
                   //chunk.name == "sRGB" || // gamma correction won't work with such chunk
                   chunk.name == "iCCP" || 
                   chunk.name == "cICP" || 
                   chunk.name == "iCCN";
        });
...
export class BlazerPNG {
    ...
    async encode(chunks, RGBdraw = ()=>{}, Adraw = null) {
        ...
        if (this.options.useNative && this.ctx) {
            this.RGBdraw();
            const _imageData_ = this.ctx.getImageData(0, 0, this.w, this.h);
            return new InjectPNG(this.chunks, {width: this.w, height: this.h}).recode( UPNG.encodeLL([_imageData_.data.buffer], _imageData_.width, _imageData_.height, 3, 1, (_imageData_.depth||8)) );
        } else {
