...
export class BlazerPNG {
    ...
    async encode(chunks, RGBdraw = ()=>{}, Adraw = null) {
        ...
        if (this.options.useNative && this.ctx) {
            this.RGBdraw();
            return new InjectPNG(this.chunks, {width: this.w, height: this.h}).recode(await blobToArrayBuffer(this.ctx.canvas.convertToBlob({type: "image/png", colorSpace: "srgb"})));
        } else {
