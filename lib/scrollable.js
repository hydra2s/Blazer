...
try {
    ...
    class ScrollBar {
        ...
        set shift(a) {
            this.percent = Math.min(Math.max((this._shift = a) / Math.max(this.diffSize, 0.0001), 0.0), 1.0);
        }
        ...
        get percentOf() {
            const _percent_ = Math.min(Math.max(this._percent, 0.0), 1.0);
            {
                const _shift = (this._shift = _percent_ * this.diffSize);
                this.track.style.setProperty("--offsetPercent", _percent_);
            }
    ...
    customElements.define("scroll-able", Scrollable = class Scrollable extends HTMLElement {
        ...
        _saveToStorage() {
            if (this.hasAttribute("storage")) {
                if (!isNaN(this._scroll[0].percent)) { localStorage.setItem(this.getAttribute("storage") + "-spX", this._scroll[0]._percent); };
                if (!isNaN(this._scroll[1].percent)) { localStorage.setItem(this.getAttribute("storage") + "-spY", this._scroll[1]._percent); };
            }
            return this;
        }
