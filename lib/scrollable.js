...
try {
    ...
    class ScrollBar {
        ...
        get shift() {
            ...
            const _percent_ = (this._percent = Math.min(Math.max(_shift_ / this.diffSize, 0.0), 1.0));
        ...
        set shift(a) {
            const _percent_ = (this._percent = Math.min(Math.max((this._shift = a) / this.diffSize, 0.0), 1.0));
            this.track.style.setProperty("--offsetPercent", _percent_);
            this._applicant(_percent_, this.diffOfBox);
        }
        ...
        constructor() {
            ...
            (async()=>{
                ...
                requestAnimationFrame(()=>{
                    this._updateSize()._loadFromStorage()._updateScroll()._saveToStorage();
                });
            })();
            ...
            //
            requestAnimationFrame(()=>{
                this._updateSize()._loadFromStorage()._updateScroll();
            });
