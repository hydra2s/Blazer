...
export class Executor {
  ...
  apply(className, methodName, args, thisArg) {
    let wsc = this.wsc;
    let cmd = this.cmd;
    return wrap(wsc.sendRequest(cmd.apply(className, methodName, args, thisArg), args.filter((pre)=>pre?.$shared).map((pre)=>pre.$data)));
  }

  construct(className, args) {
    let wsc = this.wsc;
    let cmd = this.cmd;
    return wrap(wsc.sendRequest(cmd.construct(className, args), args.filter((pre)=>pre?.$shared).map((pre)=>pre.$data)));
  }
