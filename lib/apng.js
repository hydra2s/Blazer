...
export class APNGPreRender {
    ...
    async prerender() {
        ...
        const images = await Promise.all(this.frames.map((F)=>createImageBitmap(F.blob)));
        ...
        for await (const p of P) {
            ...
            prevF = F, prevI = null;
            if (F && F.disposeOp == 2) {
                prevI = createImageBitmap(this.canvas, F.left, F.top, F.width, F.height);
            }
            if (F.blendOp == 0) { ctx.clearRect(F.left, F.top, F.width, F.height); }
            ctx.drawImage(images.shift(), F.left, F.top);
            //ctx.drawImage(await createImageBitmap(F.blob), F.left, F.top);
            Fm.push(createImageBitmap(this.canvas, ...(this.options.fullFrame ? [0, 0, this.canvas.width, this.canvas.height] : [F.left, F.top, F.width, F.height])));
        }
        return await Promise.all(Fm);
    }
}
...
export class APNGDecoder {
    ...
    async load(url) {
        ...
        console.time("Parsing APNG");
        const parsed = this.parse();
        console.timeEnd("Parsing APNG");
        return parsed;
    }
