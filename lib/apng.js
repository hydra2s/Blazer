import { swap32, createDOMCanvas, loadImage } from "./utils.js";
...
export class APNGPreRender {
    ...
    async prerender() {
        ...
        const Fm = await Promise.all(this.frames.map(async (F)=>createImageBitmap(await F.blob)));
        let prevF = null, prevI = null;

        const P = await Promise.all(this.frames.map(async (F,I)=>[{
            disposeOp: await F.disposeOp, 
            blendOp: await F.blendOp, 
            width: await F.width, 
            height: await F.height, 
            top: await F.top, 
            left: await F.left, 
        }, I]));
