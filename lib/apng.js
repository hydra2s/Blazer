...
export class APNGPreRender {
    ...
    async prerender() {
        ...
        const Fm = await Promise.all(this.frames.map((F)=>createImageBitmap(F.blob)));
        ...
        for await (const p of P) {
            ...
            Fm[I] = createImageBitmap(this.canvas, ...(this.options.fullFrame ? [0, 0, this.canvas.width, this.canvas.height] : [F.left, F.top, F.width, F.height]));
        }
        return Fm;
    }
}
