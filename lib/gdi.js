...
export class GDI {
    ...
    gen(func) {
        ...
        //
        const u16 = this.is16bitRGB || this.is16bitA;
        ...
        this.gl.uniform1i(this.gl.getUniformLocation(func["_program_"], `_shift0_`), (u16?16:8)-b16rgb);
        this.gl.uniform1i(this.gl.getUniformLocation(func["_program_"], `_shift1_`), (u16?16:8)-b16a);
...
//
uniform int _shift0_;
uniform int _shift1_;

// for PNG support...
uint swap16(in uint a) {
    return ((a&0xFFu)<<8)|((a&0xFF00u)>>8);
}

//
uint inc(in uint c, in int p) {
    return p > 0 ? swap16(c<<p) : c;
}
...
void main() {
    ...
    uint R0 = inc(_color0_.r, _shift0_), G0 = inc(_color0_.g, _shift0_), B0 = inc(_color0_.b, _shift0_), A0 = inc(_color0_.a, _shift0_);
    uint R1 = inc(_color1_.r, _shift1_), G1 = inc(_color1_.g, _shift1_), B1 = inc(_color1_.b, _shift1_), A1 = inc(_color1_.a, _shift1_);
...
`
    ...
    getImageData(x, y, width, height, options = {}) {
        ...
        const pixels = options.storage || new (u16 ? Uint16Array : Uint8Array)(width * height * 4);
        if (this.fb) { 
            this.gl.bindFramebuffer(this.gl.READ_FRAMEBUFFER, this.fb); 
            this.gl.bindTexture(this.gl.TEXTURE_2D, this.output);
        };
        this.gl.readPixels(x, y, width, height, this.gl.RGBA_INTEGER, u16 ? this.gl.UNSIGNED_SHORT : this.gl.UNSIGNED_BYTE, pixels, 0);
        return {
            width: width,
            height: height,
            data: pixels,
