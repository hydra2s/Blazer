...
class InterCom {
  ...
  observe() {
    ...
    this.connection.on("message", async (message, isBinary) => {
      ...
      let {result, exception, error, id, hasResult, hasKey, type, isOperate} = handled;
...
class InterWork {
  ...
  observe() {
    ...
    this.thread.addEventListener("message", async (message, isBinary) => {
      ...
      let handled = await this.pt.handleEvent(this.preHandle(message.data.command, shared), shared);
      let {result, exception, error, id, hasResult, hasKey, type, isOperate} = handled;

      // avoid wrong delete when used multiple InterWork
      //if (shared && (hasKey || hasResult || isOperate)) {
