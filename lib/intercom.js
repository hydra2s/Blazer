...
class InterCom {
  ...
  preHandle(cmdObj) {
    if (cmdObj.result && typeof cmdObj.result == "string") { cmdObj.result = JSOX.parse(atob(cmdObj.result)); };
    return cmdObj;
  }
    ...
    this.connection.on("message", async (message, isBinary) => {
      ...
      if (type && type != "result" && type != "error" && type != "unknown") 
      {
        ...
        await this.sendAnswer({...handled, type: isError ? "error" : "result",
          result: result ? btoa(JSOX.stringify(result)) : ""
        });
      }
    });
  }
...
class InterWork {
  ...
  async sendAnswer(cmdObj, shared) {
    let existId = await (cmdObj = await cmdObj).id;
    let transfer = (await Promise.all(shared?.map(async (data)=>{ data = await data; const _buffer_ = await data?.buffer; return ( _buffer_ instanceof ArrayBuffer ? _buffer_ : data); })||[])).filter((data)=>{
        return this.transfer.some((I)=> { return data instanceof I; });
    });
    
    this.thread.postMessage({
        command: JSON.stringify(Object.assign(cmdObj, {id: existId ? existId : uuidv4()})),
  ...
  observe() {
    ...
    this.thread.addEventListener("message", async (message, isBinary) => {
      ...
      let handled = await this.pt.handleEvent(this.preHandle(JSON.parse(message.data.command)), shared);
      let {result, exception, error, id, hasResult, hasKey, type} = handled;
