...
export class MNGImage extends MNGShow {
    ...
    draw(ctx) {
        if (!this.parent.doNotShow) {
            ctx.drawImage(this.data, this.parent.position[0] + this.position[0], this.parent.position[1] + this.position[1]);
            //ctx.drawImage(await this.data, this.parent.position[0], this.parent.position[1]);
        }
        return this;
    }

    apply(objs) {
        this.parent = objs.objects[this.id];
        objs.lastTime = performance.now();
