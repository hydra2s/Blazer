...
export class MNGLoop extends MNGProgram {
    ...
    async execute(objs) {
        objs.current = this;
        objs.nextInstruction = this.instructions[this.iterator = 0];
        if ((this.literator++) >= this.lcount) {
            objs.current = this.parent;
            this.literator = 0;
            //objs.nextInstruction = this.instructions[this.literator = 0];
        }
        return this;
    }
}
...
export class MNGShow extends MNGInstruction {
    ...
    async execute(objs) {
        ...
        const _ID_ = objs.visible.indexOf(this.id);
        if (_ID_ < 0 && this.mode) {
            objs.visible.push(this.id);
        } else
        if (_ID_ >= 0 && !this.mode) {
            objs.visible.splice(_ID_, 1);
        }
        return this;
    }
}
...
export class OpenPNG {
    ...
    async recodePNG() {
        console.log(this.reader.chunks);
        ...
        for (const C of this.reader.chunks) 
        {
            switch(C.name) {
                ...
                case "CLIP":
                    ...
                    _clip_.type = C.view.getUint8(4, false);
                    _clip_.clip = [C.view.getUint32(4, false), C.view.getUint32(8, false), C.view.getUint32(12, false), C.view.getUint32(16, false)];
                ...
                case "FRAM":
                    ...
                    _fram_.changeTimeoutAndTermination = C.view.getUint8(_I_+2);
                    _fram_.changeLayerClipping = C.view.getUint8(_I_+3);
                    _fram_.changeSyncId = C.view.getUint8(_I_+4);

                    if (C.view.byteLength >= (_I_+9)) {
                        _fram_.interframeDelay = C.view.getUint32(_I_+5, false);
                        _fram_.timeout = C.view.getUint32(_I_+9, false);
                        _fram_.layerClippingMode = C.view.getUint8(_I_+13, false);
                    }

                    if (C.view.byteLength >= (_I_+30)) {
                        _fram_.clip[0] = C.view.getUint32(_I_+14, false);
                        _fram_.clip[1] = C.view.getUint32(_I_+18, false);
                        _fram_.clip[2] = C.view.getUint32(_I_+22, false);
                        _fram_.clip[3] = C.view.getUint32(_I_+26, false);
                    }
...
export class MNGObject {
    ...
    async initial(image) {
        ...
        if (_image_) {
            if (this.canvas.width != _image_.width || this.canvas.height != _image_.height) {
...
export class MNGRenderer {
    ...
    async toNextInstruction() {
        await (this.instruction = this.nextInstruction)?.execute(this);
        this.nextInstruction = this.current.instructions[++this.current.iterator];
        if (!this.nextInstruction) { await this.current.execute(this); };
    }

    async drawObjects() {
        this.objects["_"].ctx.clearRect(0, 0, this.objects["_"].canvas.width, this.objects["_"].canvas.height);
        await this.objects["_"].initial();
        ...
        //
        if ((performance.now() - this.lastTime) >= this.frameTime || this.nextInstruction?.type != "SHOW") {
