...
export class MNGImage extends MNGInstruction {
    ...
    async show(ctx) {
        ctx.drawImage(await this.data, this.shifting[0] + this.position[0], this.shifting[1] + this.position[1]);
    }
        ...
        for (const C of this.reader.chunks) 
        {
            switch(C.name) {
                ...
                case "IHDR":
                    _skipping_ = true;
                    _image_ = new MNGImage(_defi_.id, _defi_);
                    _image_.data = createImageBitmap(await this.pngLoader.loadInternal(new DataReader(this.reader.data, C.slice.byteOffset)));
                    this.parsed.instructions.push(_image_);
                break;
    ...
    async drawObjects() {
        ...
        if (!_isNotShow_) 
        {
            ...
            if (this.frame?.clip) {
                ...
                ctx.beginPath();
                ctx.rect(0, 0, this.mng.width, this.mng.height);
                ctx.clip();
            }

            //
            ctx.clearRect(0, 0, this.mng.width, this.mng.height);

            //
            for (let V of this.visible) {
                await this.objects[V].show(ctx);
            }
