...
//
export class MNGFrame extends MNGClear {
    constructor(id = 0) {
        super(id);
        this.type = "FRAM";
...
export class MNGRenderer {
    ...
    drawObjects() {
        ...
        let _isDraw_ = (["SHOW"].indexOf(this.instruction?.type) >= 0);
        do {
            if ( _isDraw_ && (performance.now() - this.lastTime) <= Math.max(this.frame?.interframeDelay || 0, 0) ) { break; };
            this.toNextInstruction();
        }
        while (!(_isDraw_ = ["SHOW"].indexOf(this.instruction?.type) >= 0));
        return this;
    }
