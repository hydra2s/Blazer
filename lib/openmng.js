        ...
        for (const C of this.reader.chunks) 
        {
            switch(C.name) {
                ...
                case "TERM": // TODO: TERM support (needed for MNG-LC)
                    this.parsed.termination = C.view.getUint8(1, false);
                    this.parsed.lcount = C.view.getUint32(6, false);
                    // currently, not supported
                    //_term_ = C.view.getUint8(1, false);
                break;
                ...
                case "LOOP":
                    const _loop_ = new MNGLoop(this.parsed, C.view.getUint32(1, false));
                    _loop_.termination = _term_;
                    this.parsed.instructions.push(_loop_);
...
export class MNGRenderer {
    constructor(mng) {
        ...
        this.ctx = this.canvas.getContextDeepSpace("2d", { 
            desynchronized: true, 
    ...
    toNextInstruction() {
        ...
        if (this.current && !this.current.terminated) 
        {
            ...
            if (this.current.iterator >= this.current.instructions.length) {
                ...
                while ((++this.current.literator) >= this.current.lcount) {
                    if (this.current.termination == 0) {
                        this.current.terminated = true; break;
                    } else
                    {
                        this.current.literator = 0;
                    }
                    if (this.current.parent) { this.current = this.current.parent; } else { break; };
                }
            }
        }
    ...
    drawObjects() {
        ...
        do {
            ...
            if (!this.current || this.current.terminated) { break; };
        }
