    ...
    async parse() {
        ...
        for (const C of this.reader.chunks) 
        {
            switch(C.name) {
                ...
                case "DEFI":
                this.parsed.instructions.push(new MNGDefinition(C.view.getUint32(0, false)));
                break;

                case "CLON":
                this.parsed.instructions.push(new MNGClone(C.view.getUint32(4, false), C.view.getUint32(0, false)));
                break;

                case "MOVE":
                this.parsed.instructions.push(new MNGMove(C.view.getUint32(0, false)));
                break;
