...
export class MNGShow extends MNGInstruction {
    ...
    apply(objs) {
        ...
        {   // apply a MOVE chunk
            _obj_.position.x = _obj_.shifting.x;
            _obj_.position.y = _obj_.shifting.y;
        }
...
export class MNGImage extends MNGShow {
    ...
    draw(ctx) {
        if (!this.parent.doNotShow) {
            ctx.drawImage(this.data, this.parent.position.x + this.position.x, this.parent.position.y + this.position.y);
        }
        return this;
    }
    ...
    apply(objs) {
        ...
        if (ptr.images.indexOf(this) < 0) {
            this.position.x = ptr.shifting.x;
            this.position.y = ptr.shifting.y;
...
export class MNGObject extends MNGInstruction {
    ...
    apply(objs) {
        {   // TODO: correct handling initial state
            ...
            this.position.x = this.initial.x, this.position.y = this.initial.y;
