    ...
    async parse() {
        ...
        for (const C of this.reader.chunks) 
        {
            switch(C.name) {
                ...
                case "JHDR":
                    _skipping_ = true;
                    this.imagesE.push( this.jngLoader.loadInternal(new DataReader(this.reader.data, C.slice.byteOffset)));
                    (this.parsed.instructions.slice(-1)[0]?.type == "DEFI" ? this.parsed.instructions.slice(-1)[0] : this).images.push({offset: _move_.offset, data: createImageBitmap(await this.imagesE.slice(-1)[0])});
                break;

                // PNG!
                case "IHDR":
                    _skipping_ = true;
                    this.imagesE.push(this.pngLoader.loadInternal(new DataReader(this.reader.data, C.slice.byteOffset)));
                    (this.parsed.instructions.slice(-1)[0]?.type == "DEFI" ? this.parsed.instructions.slice(-1)[0] : this).images.push({offset: _move_.offset, data: createImageBitmap(await this.imagesE.slice(-1)[0])});
                break;
    ...
    async initial(resize = true) {
        ...
        for (const _obj_ of this.images) {
            ...
            if (_image_) {
                ...
                this.ctx.drawImage(_image_, _obj_.offset[0], _obj_.offset[1]);
            }
            return _image_;
        }
        return this;
...
export class MNGRenderer {
    constructor(mng) {
        ...
        this.objects["_"].images = this.mng.images;
        this.visible = [];
