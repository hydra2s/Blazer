    ...
    async parse() {
        ...
        for (const C of this.reader.chunks) 
        {
            switch(C.name) {
                ...
                case "DEFI":
                this.parsed.instructions.push(new MNGDefinition(C.view.byteLength >= 4 ? C.view.getUint16(0, false) : 1));
                break;
                ...
                case "SHOW":
                this.parsed.instructions.push(new MNGShow(C.view.getUint16(0, false)));
                break;

                case "MOVE":
                this.parsed.instructions.push(new MNGMove(C.view.getUint16(0, false)));
                break;

                case "FRAM":
                this.parsed.instructions.push(new MNGFrame(  ));
                break;
