...
export class MNGRenderer {
    ...
    async drawObjects() {
        ...
        while (this.nextInstruction && (this.nextInstruction.type != "SHOW" || ((performance.now() - this.lastTime) >= this.frameTime))) {
            await this.toNextInstruction();
            if ((performance.now() - this.lastTime) >= this.timeout) { break; };
        }
